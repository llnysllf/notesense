...
requireSnippets("package.json", [
  ...
  "npm run design:check && npm run accessibility:check && npm run testing:check && npm run browsers:check && npm run performance:check && npm run operations:check && npm run observability:check && npm run release:notes",
  '"@axe-core/playwright"',
  '"eslint-plugin-jsx-a11y"',
]);
