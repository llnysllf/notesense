...
- total Pages output: 350 KiB raw, 105 KiB gzip

Secondary statistics, song screens, and the evidence ledger are split from the initial practice route. The total raw budget includes those deferred chunks and PWA precache metadata; the 105 KiB gzip cap remains the shipped-network constraint.
